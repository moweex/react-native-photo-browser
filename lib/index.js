...
import { isIphoneXorAbove, isLandscape, getToolbarHeight } from './utils'
...
export default class PhotoBrowser extends React.Component {
  ...
  _toggleFullScreen(display: boolean) {
    ...
    Animated.timing(this.state.fullScreenAnim, {
      toValue: display ? 1 : 0,
      duration: 300
    }).start();
  }
  ...
  render() {
    ...
    if (mediaList.length > 0) {
          ...
          <Animated.View
            style={{
              ...
              marginTop: fullScreenAnim.interpolate({
                inputRange: [0, 1],
                outputRange: [0, screenHeight * -1 - getToolbarHeight()]
              })
            }}
          >
            <GridContainer
      ...
      fullScreenContainer = (
        <FullScreenContainer
          ...
          indexChanged={index => this.props.indexChanged(index)}
        />
      );
    }
    ...
    let topBarHeight = isIphoneXorAbove() && !isLandscape() ? 90 : isIphoneXorAbove() && isLandscape() ? 50 : 60
    ...
    return (
      <View
        ...
        style={[
          styles.container,
          {
            paddingTop: gridContainer ? getToolbarHeight() : 0,
            paddingHorizontal: isLandscape() && isIphoneXorAbove() ? 35 : 0
          },
          style
        ]}
      >
