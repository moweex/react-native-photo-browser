...
export default class FullScreenContainer extends React.Component {
  ...
  componentDidMount() {
    DeviceEventEmitter.addListener("didUpdateDimensions", () => {
      ...
      if (this.childPhotoRefs[this.state.currentIndex]) {
        console.log("didUpdateDimensions",this.state.currentIndex)
  ...
  _onPageSelected(page) {
    ...
    if (currentIndex !== newIndex) {
      // console.log("_onPageSelected", this.childPhotoRefs);
      console.log("_onPageSelected currentIndex", currentIndex);
      ...
      <View key={`row_${rowID}`} style={styles.flex}>
        ...
        >
          <Photo
            ...
            // currentRef={this.photoRefs[rowID] ? this.photoRefs[rowID] : null}
            // ref={ref => this.setChildRefFromChild(ref)}
            lazyLoad
            // retrunRef={this.setChildRefFromChild}
