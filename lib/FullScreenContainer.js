...
export default class FullScreenContainer extends React.Component {
  ...
  componentWillUnmount() {
    this.orientationEventListener = Dimensions.removeEventListener(
      "didUpdateDimensions"
    );
  }
  ...
  setChildRef(ref) {
    let id = ref.props.id;
    this.childPhotoRefs[id] = ref;
    console.log("Image Refs", ref);
  }

  resetZoomScale(imageIndex) {
    let index = imageIndex ? imageIndex : this.state.currentIndex;
    ...
    for (let i = 0, len = this.childPhotoRefs.length; i < len; i++) {
      if (i !== this.state.imageIndex && this.childPhotoRefs[i]) {
