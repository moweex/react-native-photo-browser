...
export default class Photo extends Component {
  ...
  componentWillMount() {
    ...
    ImageCacheManager()
      ...
      .then(cachedFilePath => {
        reactotron.logImportant("cachedFilePath", cachedFilePath);
        this.setState({
  ...
  updateDeviceWidth(e) {
    reactotron.log("EVENT updateDeviceWidth", e);
    const layoutWidth = Dimensions.get("window").width;
    reactotron.log("layoutWidth", layoutWidth);
  ...
  _onProgress(event) {
    ...
    reactotron.logImportant("progress", progress);
    reactotron.logImportant("this.props.id", this.props);
  ...
  _renderProgressIndicator() {
    ...
    return (
      <ProgressElement progress={progress} thickness={50} color={"#616161"} />
    );
    return null;
  }
  ...
  render() {
    ...
    return (
      <View style={[styles.container, sizeStyle]}>
        {error ? this._renderErrorIcon() : null}
        {progress !== 1 ? (
          <View
            style={{
              position: "absolute",
